...
Inherits Canvas
	...
	#tag Method, Flags = &h0
		Sub Scale()
		  ...
		  Else
		    ...
		    buffer = GetFIPic(fi.Rescale(newSize.Left.Int32Value, newSize.Right.Int32Value))
		  End If
	...
	#tag Method, Flags = &h0
		Function SelectedPics() As Picture()
		  Dim p() As Picture
		  For i As Integer = 0 To UBound(Objects)
		    If Objects(i).Hilight Then
		      p.Append(Objects(i).image)
		    End If
		  Next
		  Return p
		End Function
	...
	#tag Method, Flags = &h0
		Function SelectionEnd(Action As Integer) As Picture
		  Dim X, Y As Integer
		  X = Abs(DragStartX - DragEndX)
		  Y = Abs(DragStartY - DragEndY)
		  Dim p As Picture = NewBuffer(X, Y, 32)
		  
		  Select Case Action
		  Case Select_Mode_To_Buffer
		    If p <> Nil Then
		      SaveUndo()
		      p.Graphics.DrawPicture(Buffer, 0, 0, Buffer.Width, Buffer.Height, DragStartX, DragStartY, Buffer.Width, Buffer.Height)
		      Buffer.Graphics.FillRect(DragStartX, DragStartY, X, Y)
		    End If
		  Case Select_Mode_Copy
		    Dim cb As New Clipboard
		    'p.Graphics.DrawPicture(Buffer, 0, 0, Buffer.Width, Buffer.Height, DragStartX, DragStartY, Buffer.Width, Buffer.Height)
		    cb.Picture = CopyRegion(DragStartX, DragStartY, Buffer.Width - DragStartX, Buffer.Height - DragStartY)
		  Case Select_Mode_Cut
		    SaveUndo()
		    Dim cb As New Clipboard
		    p.Graphics.DrawPicture(Buffer, 0, 0, Buffer.Width, Buffer.Height, DragStartX, DragStartY, Buffer.Width, Buffer.Height)
		    cb.Picture = p
		    Buffer.Graphics.FillRect(DragStartX, DragStartY, X, Y)
		  Case Select_Mode_Delete
		    SaveUndo()
		    p = NewBuffer(p.Width, p.Height, 32)
		    p.Graphics.FillRect(0, 0, p.Width, p.Height)
		    Buffer.Graphics.DrawPicture(Buffer, DragStartX, DragStartY, p.Width, p.Height, 0, 0, p.Width, p.Height)
		  End Select
		  If Me.Mode <> Select_Mode_To_Buffer Then
		...
		Exception err As OutOfBoundsException
		  MsgBox("An impossible region was selected!")
		  Mode = MainWindow.CurrentMode
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Sub SelectRegion(X As Integer, Y As Integer, Width As Integer, Height As Integer)
		  Overlay.Graphics.ForeColor = &c00FF59
		  Overlay.Graphics.PenWidth = 1
		  Overlay.Graphics.PenHeight = 1
		  Overlay.Graphics.DrawRect(X, Y, Width, Height)
		  Overlay.Graphics.ForeColor = &cCCCCCC
		  Overlay.Graphics.FillRect(X + Width, Y + Height, 5, 5)
		  Overlay.Graphics.FillRect(X, Y, -5, -5)
		  Overlay.Graphics.FillRect(X + Width, Y, 5, -5)
		  Overlay.Graphics.FillRect(X, Y + Height, -5, 5)
		  Refresh(False)
	...
	#tag Method, Flags = &h0
		Sub ShowRedoStack()
		  ...
		  Dim i As String = SV.ShowMe(Redos, 0)
		  If i <> "" Then
		    Dim p As Picture = GetFramePicture(i)
		    Replace(p)
		  End If
		  MainWindow.TheToolsWin.LineSize.ListIndex = LineWidth
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub ShowUndoStack()
		  Dim SV As New StackViewer
		  Dim i As String = SV.ShowMe(Undos, 1)
		  If i <> "" Then
		    SaveRedo()
		    Dim p As Picture = GetFramePicture(i)
		    Replace(p)
