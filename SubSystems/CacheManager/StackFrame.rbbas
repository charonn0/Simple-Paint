...
Protected Class StackFrame
	...
	#tag Method, Flags = &h0
		Sub CopyTo(f As FolderItem, PictureFormat As Integer = -1, Quality As Integer = -1)
		  Me.Data.Save(f, PictureFormat, Quality)
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub Destructor()
		  ThePicture = Nil
		  Unlock()
		  SpecialFolder.Temporary.Child("Simple Paint").Child(Key).Delete
		End Sub
	#tag EndMethod
	...
	#tag ComputedProperty, Flags = &h0
		...
		#tag Setter
			Set
			  If value Then
			    ThePicture = Nil
			    Lock(SpecialFolder.Temporary.Child("Simple Paint").Child(Key))
			  End If
			  mPaged = value
			End Set
		#tag EndSetter
