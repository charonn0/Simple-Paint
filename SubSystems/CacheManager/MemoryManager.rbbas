...
Protected Module MemoryManager
	...
	#tag Method, Flags = &h1
		Protected Function ExtractIcons() As Boolean
		  Debug("Create cache")
		  Icons = New Dictionary
		  Loading.ShowMe("Initialiazing...")
		  Dim f As FolderItem = SpecialFolder.Temporary.Child("Simple Paint")
		  f.CreateAsFolder()
	...
	#tag Method, Flags = &h1
		Protected Function FlushCache() As Boolean
		  ...
		  For i As Integer = 0 To c - 1
		    ...
		    Dim f As FolderItem = SpecialFolder.Temporary.Child("Simple Paint").Child(n)
		    Try
	...
	#tag Method, Flags = &h1
		Protected Function ForceTrim(Key As String) As Boolean
		  ...
		  If sf.Paged Then
		    ...
		    Return True
		  End If
		  Dim f As FolderItem = SpecialFolder.Temporary.Child("Simple Paint").Child(Key)
		  Try
		    #pragma BreakOnExceptions Off
		...
		Protected Sub TrimCache(Sender As Timer)
		  ...
		  If usedSpace > MaxMem Then
		    ...
		    For i As Integer = 0 To c - 1
		      ...
		      If d.TotalSeconds - sf.TimeStamp < 5000 Then//5 seconds for testing
		        Dim f As FolderItem = SpecialFolder.Temporary.Child("Simple Paint").Child(n)
		        Try
		          #pragma BreakOnExceptions Off
		          GetFramePicture(n).Save(f, Picture.SaveAsPNG)
