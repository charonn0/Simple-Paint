...
Protected Module File_Ops
	...
	#tag Method, Flags = &h0
		Function CreateURLShortcut(scTarget as String, scName as String) As FolderItem
		  Dim f As FolderItem = SpecialFolder.Temporary.Child(scName + ".url")
		  Dim tos As TextOutputStream
		  tos = tos.Create(f)
		  tos.WriteLine("[InternetShortcut]")
		  tos.WriteLine("URL=" + scTarget)
		  Return f
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub StopWatchingDirectory(WatchHandle As Integer)
		  ...
		  '#pragma Warning "Fixme: This needs a complete overhaul."
		  #If TargetWin32 Then
